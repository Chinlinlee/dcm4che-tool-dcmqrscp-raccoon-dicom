...
class SeriesQueryTask extends StudyQueryTask {
    ...
    protected MongoSeries mongoSeriesRec;
    protected MongoDbReader mongoDbReader;
    protected MongoCursor<Document> seriesCursor;
    ...
    protected void getSeries() {
        if (seriesCursor.hasNext()) {
            Document doc = seriesCursor.next();
            mongoSeriesRec = new MongoSeries(mongoStudyRec.patient, mongoStudyRec.study, doc);
            seriesRec = mongoSeriesRec.getAttributes();
        } else {
            seriesRec = null;
        }
    }

    protected void getNextSeriesCursor() {
        Attributes queryAttr = new Attributes();
        queryAttr.addAll(keys, true);
        queryAttr.addSelected(studyRec, Tag.StudyInstanceUID);
        queryAttr.addSelected(studyRec, Tag.PatientID);
        this.seriesCursor = this.mongoDbReader.findSeriesRecord(queryAttr);
    }
